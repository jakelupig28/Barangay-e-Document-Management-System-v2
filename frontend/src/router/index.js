...
const routes = [
  ...
  { path: '/privacy', name: 'privacy', component: () => import('@/views/PrivacyPolicy.vue'), meta: { title: 'Privacy Policy' } },
  { path: '/terms', name: 'terms', component: () => import('@/views/TermsOfService.vue'), meta: { title: 'Terms of Service' } },
  ...
  },
  {
    ...
    component: () => import('@/components/resident/ResidentMessages.vue'),
    meta: { requiresAuth: true, allowedRoles: ['resident'], title: 'Resident Messages' }
  },
  ...
  },
  {
    ...
    component: () => import('@/components/official/OfficialMessages.vue'),
    meta: { requiresAuth: true, allowedRoles: ['barangay_staff'], title: 'Barangay Staff Messages' }
  },
{
...
router.beforeEach(async (to, from, next) => {
  ...
  const requiresOfficial = to.matched.some(record => record.meta.requiresOfficial);
  ...
  if (requiresAuth && currentUser && currentUser.role === 'resident') {
    ...
    if (residentRecord && residentRecord.status !== 'approved' && residentRecord.status !== 'active' && to.name !== 'resident-dashboard') {
      return next({ name: 'resident-dashboard' });
    }
  }
  ...
  if (requiresAuth && requiresOfficial) {
    const currentRole = currentUser?.role;
    if (currentRole !== 'barangay_staff' && currentRole !== 'super_admin') {
      return next(getDashboardRoute(currentRole));
    }
  }
