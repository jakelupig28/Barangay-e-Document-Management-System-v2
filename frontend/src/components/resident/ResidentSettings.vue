    ...
    async loadProfile() {
      try {
        ...
        if (this.isFirebaseReady()) {
          ...
          const residentRecord = dbData.residents?.find(r => r.userId === uid) || dbData.users?.find(u => u.id === uid)
          if (residentRecord) {
            this.profile = { 
              ...residentRecord, 
              email: residentRecord.email || user.email,
              profile: residentRecord.profile || {}
            }
          }
        }
      } catch (error) {
    ...
    async loadSettings() {
      try {
        ...
        if (this.isFirebaseReady()) {
          ...
          const dbData = localDb.readDb()
          const localSettings = dbData.user_settings?.[uid]
          if (localSettings) {
    ...
    async saveSettings() {
      ...
      try {
        ...
        if (this.isFirebaseReady()) {
          ...
          const dbData = localDb.readDb()
          if (!dbData.user_settings) dbData.user_settings = {}
          dbData.user_settings[uid] = {
            ...dbData.user_settings[uid],
            ...this.settings
          }
    ...
    async downloadData() {
      try {
        ...
        if (this.isFirebaseReady()) {
          ...
          const qReq = query(requestsRef, where('userId', '==', uid))
          const reqSnap = await getDocs(qReq)
          requestsList = reqSnap.docs.map(d => ({ id: d.id, ...d.data() }))
          
          // Fetch reports
          const reportsRef = collection(db, 'reports')
          const qRep = query(reportsRef, where('userId', '==', uid))
          const repSnap = await getDocs(qRep)
          reportsList = repSnap.docs.map(d => ({ id: d.id, ...d.data() }))
        } else {
          const dbData = localDb.readDb()
          requestsList = (dbData.requests || []).filter(r => r.userId === uid)
          reportsList = (dbData.reports || []).filter(r => r.userId === uid)
        }
        ...
        // Tab 1: Profile Info
        const profileData = [
          { "Field": "Resident ID", "Value": this.profile?.profile?.residentId || this.profile?.residentId || 'N/A' },
          { "Field": "Full Name", "Value": this.profile?.profile?.name || this.profile?.name || 'N/A' },
          { "Field": "Email Address", "Value": this.profile?.email || 'N/A' },
          { "Field": "Contact Number", "Value": this.profile?.profile?.contact || this.profile?.contact || 'N/A' },
          { "Field": "Home Address", "Value": this.profile?.profile?.address || this.profile?.address || 'N/A' },
          { "Field": "Birthdate", "Value": this.profile?.profile?.birthdate || this.profile?.birthdate || 'N/A' },
          { "Field": "Gender", "Value": this.profile?.profile?.gender || this.profile?.gender || 'N/A' },
          { "Field": "Age", "Value": this.profile?.profile?.age || this.profile?.age || 'N/A' },
          { "Field": "Civil Status", "Value": this.profile?.profile?.civilStatus || this.profile?.civilStatus || 'N/A' },
          { "Field": "Registration Status", "Value": this.profile?.status || 'N/A' },
          { "Field": "Date Registered", "Value": this.profile?.createdAt ? new Date(this.profile.createdAt).toLocaleDateString() : 'N/A' },
          { "Field": "Date Approved", "Value": this.profile?.approvedAt ? new Date(this.profile.approvedAt).toLocaleDateString() : 'N/A' }
        ]
        const profileWs = XLSX.utils.json_to_sheet(profileData)
        XLSX.utils.book_append_sheet(wb, profileWs, "Profile Info")
        
        // Tab 2: Document Requests
        const requestsRows = requestsList.map(r => ({
          "Request ID": r.id,
          "Document Type": r.documentType || r.type || 'N/A',
          "Purpose": r.purpose || 'N/A',
          "Notes": r.notes || 'None',
          "Status": (r.status || 'Pending').toUpperCase(),
          "Request Date": r.createdAt ? new Date(r.createdAt).toLocaleString() : 'N/A',
          "Rejection Reason": r.rejectionMessage || 'N/A',
          "Claim Date": r.claimedAt ? new Date(r.claimedAt).toLocaleString() : 'N/A'
        }))
        const requestsWs = XLSX.utils.json_to_sheet(requestsRows)
        XLSX.utils.book_append_sheet(wb, requestsWs, "Document Requests")
        
        // Tab 3: Reports & Incidents
        const reportsRows = reportsList.map(rep => ({
          "Report ID": rep.id,
          "Category": rep.category || 'N/A',
          "Title": rep.title || 'N/A',
          "Description": rep.description || 'N/A',
          "Status": (rep.status || 'Pending').toUpperCase(),
          "Date Reported": rep.createdAt ? new Date(rep.createdAt).toLocaleString() : 'N/A',
          "Resolution Notes": rep.resolutionNotes || 'N/A',
          "Date Resolved": rep.resolvedAt ? new Date(rep.resolvedAt).toLocaleString() : 'N/A'
        }))
        const reportsWs = XLSX.utils.json_to_sheet(reportsRows)
        XLSX.utils.book_append_sheet(wb, reportsWs, "Reports & Incidents")
        
        // 3. Download the file
        const name = this.profile?.profile?.name || this.profile?.name || 'resident'
        const sanitizedName = name.replace(/[^a-z0-9]/gi, '_').toLowerCase()
        XLSX.writeFile(wb, `barangay-data-${sanitizedName}.xlsx`)
        this.showMessage('Data exported to Excel!', 'success')
      } catch (error) {
