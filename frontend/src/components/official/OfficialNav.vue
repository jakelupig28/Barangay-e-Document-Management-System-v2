        ...
        <ul class="nav-list">
          ...
          <li class="nav-item">
            <router-link class="nav-link" active-class="active" to="/official/residents">
              <i class="nav-icon fas fa-users position-relative">
                <span v-if="pendingResidents > 0 && isCollapsed" class="position-absolute top-0 start-100 translate-middle p-1 bg-danger border border-light rounded-circle" style="width: 10px; height: 10px;">
                  <span class="visually-hidden">New alerts</span>
                </span>
              </i>
              <span class="nav-text d-flex justify-content-between align-items-center flex-grow-1">
                Residents
                <span v-if="pendingResidents > 0" class="badge bg-danger rounded-pill">{{ pendingResidents }}</span>
              </span>
            </router-link>
          </li>
          ...
          <li class="nav-item">
            <router-link class="nav-link" active-class="active" to="/official/requests">
              <i class="nav-icon fas fa-clipboard-list position-relative">
                <span v-if="newRequests > 0 && isCollapsed" class="position-absolute top-0 start-100 translate-middle p-1 bg-danger border border-light rounded-circle" style="width: 10px; height: 10px;">
                  <span class="visually-hidden">New alerts</span>
                </span>
              </i>
              <span class="nav-text d-flex justify-content-between align-items-center flex-grow-1">
                Requests
                <span v-if="newRequests > 0" class="badge bg-danger rounded-pill">{{ newRequests }}</span>
              </span>
            </router-link>
          </li>
          ...
          <!-- Messages -->
          <li class="nav-item">
            <router-link class="nav-link" active-class="active" to="/official/messages">
              <i class="nav-icon fas fa-envelope position-relative">
                <span v-if="newMessages > 0 && isCollapsed" class="position-absolute top-0 start-100 translate-middle p-1 bg-danger border border-light rounded-circle" style="width: 10px; height: 10px;">
                  <span class="visually-hidden">New messages</span>
                </span>
              </i>
              <span class="nav-text d-flex justify-content-between align-items-center flex-grow-1">
                Messages
                <span v-if="newMessages > 0" class="badge bg-danger rounded-pill">{{ newMessages }}</span>
              </span>
            </router-link>
          </li>
...
export default {
  ...
  setup() {
    ...
    const pendingResidents = ref(0);
    const newRequests = ref(0);
    const newMessages = ref(0);
    let intervalId = null;

    const checkCounts = () => {
      pendingResidents.value = localDb.getPendingResidents().length;
      const db = localDb.readDb();
      newRequests.value = (db.requests || []).filter(r => r.status === 'pending').length;
      newMessages.value = (db.messages || []).filter(msg => msg.senderRole === 'resident' && !msg.isRead).length;
    };

    onMounted(() => {
      checkCounts();
      intervalId = setInterval(checkCounts, 2000);
    ...
    return { logout, isCollapsed, toggleSidebar, pendingResidents, newRequests, newMessages };
  },
};
</script>
