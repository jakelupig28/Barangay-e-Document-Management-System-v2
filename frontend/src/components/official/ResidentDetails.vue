    ...
    <div v-else-if="resident" class="resident-content">
      ...
      <div class="mt-4" v-if="resident.barangayIdImage">
        <div class="card border-0 shadow-sm">
          ...
          <div class="card-body p-4">
            <div v-if="resident.previousBarangayIdImage" class="row g-4">
              <div class="col-md-6 text-center border-end">
                <h6 class="mb-3 text-muted fw-bold text-uppercase"><i class="fas fa-history me-1"></i> ID Tagged for Compliance</h6>
                <div class="image-preview-wrapper p-2 bg-light rounded border">
                  <img
                    :src="resident.previousBarangayIdImage"
                    alt="Previous Barangay ID"
                    class="img-fluid rounded shadow-sm uploaded-id-preview"
                    style="max-height: 350px; object-fit: contain; cursor: pointer;"
                    @click="openImagePreview(resident.previousBarangayIdImage)"
                  >
                </div>
                <div class="small text-muted mt-2">Click the ID to view a larger version</div>
              </div>
              <div class="col-md-6 text-center">
                <h6 class="mb-3 text-primary fw-bold text-uppercase"><i class="fas fa-id-card me-1"></i> Re-uploaded ID by Resident</h6>
                <div class="image-preview-wrapper p-2 bg-light rounded border">
                  <img
                    ...
                    class="img-fluid rounded shadow-sm uploaded-id-preview"
                    style="max-height: 350px; object-fit: contain; cursor: pointer;"
                    @click="openImagePreview(resident.barangayIdImage)"
                  >
                </div>
                <div class="small text-muted mt-2">Click the ID to view a larger version</div>
              </div>
      ...
      <div class="d-flex flex-wrap gap-3 mt-4">
        <template v-if="!resident.isApproved && resident.status !== 'approved' && resident.status !== 'active'">
          <button class="btn btn-success px-4 action-btn" @click="approveResident">
            <i class="fas fa-check me-2"></i> Approve Resident
          </button>
          <button class="btn btn-warning px-4 action-btn" @click="tagCompliance">
            <i class="fas fa-exclamation-circle me-2"></i> Request Compliance
          </button>
          <button class="btn btn-danger px-4 action-btn" @click="rejectResident">
  ...
  methods: {
      ...
      try {
        ...
        if (!this.isFirebaseReady()) {
          ...
          if (user) {
            this.resident = {
              ...
              isApproved: user.status === 'active' || user.status === 'approved' || (residentObj && (residentObj.status === 'active' || residentObj.status === 'approved')) || user.role !== 'resident',
              ...
              previousBarangayIdImage: (residentObj && residentObj.previousBarangayIdImage) || user.previousBarangayIdImage || null,
        ...
        if (docSnap.exists()) {
          ...
          this.resident = {
            ...
            barangayIdImage: data.barangayIdImage || null,
            previousBarangayIdImage: data.previousBarangayIdImage || null,
    ...
    async tagCompliance() {
      ...
      try {
        ...
        console.error('Error tagging compliance:', error)
    ...
    openImagePreview(imgUrl) {
      this.previewImageUrl = imgUrl || this.resident?.barangayIdImage
