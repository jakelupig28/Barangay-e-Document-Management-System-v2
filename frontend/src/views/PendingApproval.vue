  ...
  <div class="container py-5" style="max-width: 720px;">
    <div class="card shadow-sm">
      <div class="card-body p-4 text-center">
        ...
        <button v-if="status === 'approved' || status === 'active'" class="btn btn-success mt-2" @click="$router.push({ name: 'resident-dashboard' })">
...
export default {
  ...
  computed: {
    statusLabel() {
      if (this.status === 'pending_validation') return 'Pending';
      if (this.status === 'approved' || this.status === 'active') return 'Active';
      return this.status.charAt(0).toUpperCase() + this.status.slice(1);
    },
    statusClass() {
      if (this.status === 'approved' || this.status === 'active') return 'text-success';
