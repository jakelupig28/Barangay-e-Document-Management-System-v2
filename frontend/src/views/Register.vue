  ...
  <div class="auth-wrapper" :class="{ 'dark-mode': isDarkMode }">
    ...
    <div v-if="showSuccessModal" class="custom-modal-overlay">
      <div class="custom-modal">
        ...
        <p class="modal-message">Your account will be under validation by the Barangay Staff. Please wait for a confirmation message via email. Thank you.</p>
  ...
  methods: {
    ...
    async registerResident() {
      ...
      try {
        ...
        // Send registration notification email to staff
        try {
          fetch('/api/notify-registration', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              residentEmail: this.form.email,
              residentName: combinedName,
            }),
          }).catch((err) => console.warn('Email notification error:', err));
        } catch (mailErr) {
          console.warn('Failed to dispatch registration email:', mailErr);
        }
...
.auth-wrapper.dark-mode .fixed-label {
  color: #60a5fa;
}
...
.auth-wrapper.dark-mode select.modern-input {
...
.auth-wrapper.dark-mode select.modern-input option {
  background: #0f172a;
  color: #f8fafc;
}

.auth-wrapper.dark-mode .upload-label {
  color: #94a3b8 !important;
}

.auth-wrapper.dark-mode .file-drop-area {
  border-color: #334155;
  background-color: #0f172a;
}

.auth-wrapper.dark-mode .file-drop-area.has-image {
  border-color: #3b82f6;
  background-color: rgba(30, 58, 138, 0.2);
}

.auth-wrapper.dark-mode .file-message {
  color: #64748b;
}

.auth-wrapper.dark-mode .text-center p.text-muted {
  color: #64748b !important;
}

.auth-wrapper.dark-mode .login-link {
  color: #60a5fa;
}

.auth-wrapper.dark-mode .custom-modal {
  background: #1e293b;
  border: 1px solid rgba(255, 255, 255, 0.05);
}

.auth-wrapper.dark-mode .modal-title {
  color: #f8fafc;
}

.auth-wrapper.dark-mode .modal-message {
  color: #94a3b8;
}
